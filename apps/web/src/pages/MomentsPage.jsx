...
const MomentsPage = () => {
  ...
  const isUserVerified = (moment) => !!usersMap[moment.userId]?.isVerified;
                ...
                <div key={moment.id} className="bg-card rounded-2xl border border-border overflow-hidden">
                  ...
                  <div className="flex items-center justify-between px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-foreground text-sm flex items-center gap-1">{getUserName(moment)}{isUserVerified(moment) && <VerifiedBadge size={14} />}</p>
                  ...
                  <div className="px-4 pt-3 pb-2">
                    ...
                    {moment.caption && (
                      <p className="text-foreground text-sm mb-1">
                        <span className="font-semibold mr-1.5">{getUserName(moment)}</span>{isUserVerified(moment) && <VerifiedBadge size={12} className="mr-1" />}
                        {moment.caption}
                      </p>
                    )}
                            ...
                            <div key={comment.id} className="flex gap-2">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-sm text-foreground">
                                  <span className="font-semibold mr-1.5">{usersMap[comment.userId]?.name || 'Anonim'}</span>{usersMap[comment.userId]?.isVerified && <VerifiedBadge size={12} className="mr-1" />}
                                  {comment.content}
                                </p>
